...
- [ ] **T09** The shipped water declaration states `drawn = true`, `occludes = false`, `targetable = true`; dirt, grass and stone state none of the three and default to their `solid = true` — `content/base/blocks/water.luau` (verified: today it states `name`, `texture`, `solid = false`, `breakable = false`, `replaceable = true`, and its header explains `solid = false` as what "keeps the mesher from culling against it" — a sentence this task falsifies and rewrites)
      ...
      **Conductor amendment, measured in phase 2 and it lands on this task.** Two facts arrive here that no earlier task could state. **(1) The engine rule's third clause has never yet fired on shipped content, and this is where that stops being true.** Phase 2 measured it: with the clause deleted, `scene_contract`, `replay_world` and all four goldens stayed green, so on today's content `occludes(beyond)` culls first every time. The moment water declares `drawn = true`, the sea is exactly the case where the first two clauses hold and the third decides. **So if a later mutation deletes the clause and the goldens move, that is the rule starting to matter — not something breaking.** **(2) Re-baseline `crates/mc-world/benches/meshing.rs` on the tree this phase opens on, before water is declared.** Phase 2 left the mesher measurably slower (PRO-959), and without a baseline taken first, water's cost and that regression land in the same numbers and are inseparable forever.

- [ ] **T10** The shipped world's per-block face areas derived from an independent per-voxel walk rather than snapshotted from a run of the mesher, and its quad count committed as a tripwire — `crates/mc-sim/tests/support/oracle.rs`, `crates/mc-sim/tests/scene_contract.rs`, `crates/mc-sim/tests/replay_world.rs`, `crates/mc-sim/src/replay/contract.rs:28`
      Scenarios: FR-2.5-S1, FR-2.6-S1, FR-2.6-S2, FR-2.6-S3
      Depends on: T09, T04 (the walk asks `is_drawn_at`, not `is_solid_at`)
      `SCENE_QUAD_COUNT` (verified `= 2759` today) is minted **only after** the area assertions are green, per the constant's own doc comment, and **never edited to reach green** — minting it first and then deriving the walk to match inverts the only ordering that makes either meaningful. **The count is not derivable and this task must not try**: it is the scanline-greedy decomposition's answer, not the geometry's, so a walk agreeing with it would have to repeat the sweep's ordering choices and would be a copy of its subject. FR-2.6-S3 was amended on 2026-08-23 to say so; the area half of this task is unchanged and is still derived. **Do not copy FR-2.5-S1's `4 095` grass / `1` stone from the spec: re-measure it.** `replay_world.rs`'s `no_quad_of_the_meshed_replay_names_the_block_that_fills_its_sea` inverts here — it is the test author's to replace, and its positive control (a column below sea level exists before the quad count is asserted) is what made it non-vacuous and survives.
...
- [ ] **T15** A second narrow trait over the same type, and a second bitset written where solidity already is: `Targetable { fn is_targetable(&self, at: BlockPos) -> bool }`; `SolidVoxels` renamed `ResolvedVoxels` and gaining a second `Bitset`; `LastResolved` caching the pair rather than the bool; `targeted` taking `&dyn Targetable` — `crates/mc-sim/src/replay/solid.rs`, `crates/mc-sim/src/world/mod.rs:232` and `:262` (the two write sites), `crates/mc-sim/src/world/action/trace.rs:56`
      ...
      **Conductor amendment: this phase inherits a helper widening phase 2 deliberately declined.** `registry_declaring(blocks: &[(&str, bool)])` in `crates/mc-sim/tests/support/volume.rs` still takes one boolean per block, so it cannot express a fixture where `targetable` differs from `solid`. Phase 2 widened only `mesh_common`'s copy and left this one alone **because nothing before this task could state such a fixture and observe anything** — that was a deliberate refusal to generalise speculatively, not an oversight. It is this phase's test author's to widen, and the measured population of signatures exposing a solidity boolean must be re-run rather than carried: it read 21 before phase 1 and 24 after, because phase 1's own adaptation grew the set it had been measured against.
...
- [x] **T19** `DeclaredBehaviour` gains `targetable` and `BEHAVIOUR_REVISION` moves 1 → 2; `DeclaredAppearance` gains `drawn` and `occludes` and `APPEARANCE_REVISION` moves 2 → 3 — `crates/mc-world/src/persistence/format.rs:275` (verified `BEHAVIOUR_REVISION: u8 = 1`), `:276` (verified `APPEARANCE_REVISION: u8 = 2`), `:341`, `:362`
...
- [x] **T20** The whole verdict a save is opened under, asserted whole rather than as an absence — `crates/mc-world/src/persistence/table.rs:54` (`RegistryVerdict`), `crates/mc-world/tests/shipped_declarations_and_an_older_save.rs`, `crates/mc-client/tests/changed_blocks_named_on_the_error_stream.rs`
...
- [x] **T21** The reload's geometry key learns `drawn` and `occludes` and stays ignorant of `targetable`: `drawn_of` moves from `(is_solid, &textures)` to `(drawn, occludes, &textures)` — `crates/mc-sim/src/world/reload.rs:83-89` (verified: `fn drawn_of(...) -> BTreeMap<&BlockName, (bool, &FaceTextures)>` mapping `(declared.is_solid, &declared.textures)`)
...
- [x] **T22** Documentation owed by this phase — `docs/technical/world-format.md`, `docs/user/gameplay.md:68-70`
      Scenarios: —
      Depends on: T19, T20
...
### Deferred, from the conductor — five redundant empty-mesh guards

`some_quads`'s empty-mesh guard fires ahead of a comparison and prints neither
side, so where the expected value is a **non-empty literal** it can only hide
both halves of a failure — an empty actual already fails against a non-empty
expectation. Phase 2 dropped it where that applied and kept the five where it is
load-bearing (two meshes compared to each other are equal when both are empty; a
`quads.len() * AT_MOST_HALF <= visible` bound is satisfied by no quads at all).

Five more sit in `crates/mc-world/tests/mesh_neighbours.rs` — `:169`, `:229`,
`:249`, `:274`, `:298` — and all five compare against a non-empty expectation,
so the guard is redundant there in exactly the way it was redundant in phase 2's
own tests. Verified by reading each: `:169` and `:298` against six-element
helpers, `:229` and `:249` against single-element `vec![…]`, `:274` against a
three-element one. **Harmless until one of them goes red on an empty mesh, at
which point it hides both sides the way it hid phase 2's.** No phase-2 scenario
touches that file, so it was not this spec's to change.
...
### Phase 3 — the T09 bench baseline, and what the instrument cannot see

Taken **before** water was declared, on the tree the phase opens on (`bc7541a`),
per T09's standing instruction. `cargo bench -p mc-world --bench meshing`, three
consecutive rounds, criterion point estimates with their intervals in µs:

| fixture | round 1 | round 2 | round 3 | median |
|---|---|---|---|---|
| terrain | 159.73 [153.51, 166.48] | 158.21 [152.86, 164.28] | 186.20 [178.29, 194.58] | **159.7** |
| solid | 180.24 [174.30, 186.49] | 176.82 [170.57, 183.68] | 205.64 [196.55, 214.83] | **180.2** |
| checkerboard | 460.60 [445.33, 477.64] | 561.84 [551.09, 572.64] | 460.56 [447.50, 474.12] | **460.6** |

Round 3 is high on all three fixtures together and round 2's checkerboard is
high alone, which is machine noise rather than signal — rounds 1 and 3 agree on
checkerboard to 0.04 µs while disagreeing on terrain by 26 µs. The medians sit
within a few µs of the figures recorded for `281ff43` above (terrain 160.2,
solid 183.9), which is the expected reading: this tree carries that mesher
unchanged.

**The verdict line reproduced its recorded 1.6× spread, on one unchanged tree:**
149.625 µs · 221.946 µs · 155.616 µs, the middle one printing *"over its 200 µs
budget"*. The budget was not breached. This is the second independent
observation of the effect finding 2 above records, and it is why that finding
says to read criterion's interval and treat the printed verdict as a smoke
alarm.

#### The hazard this baseline was taken against is not one this bench can measure

Recorded because acting on the instruction without stating this would leave the
next reader believing a comparison exists that does not.

The instruction's premise is that *"water adds quads to the shipped scene, so
water's cost and the rebuild's cost land in the same measurement"*. **They do
not land in this measurement.** All three fixtures are built from
`fixture_registry()` in `crates/mc-world/benches/support/fixtures.rs`, an
in-memory registry of exactly two blocks assembled in Rust. The bench never
reads `content/base/`, so the shipped water declaration cannot move any number
above, and re-running it after T09 will compare two runs of identical work.

The baseline is still worth having, and is kept for what it actually anchors:
the pre-water tree's own figures, taken by the instrument PRO-959 is tracked
with, so that spec's regression has a third reading on a third day.

**What has no instrument is the thing the premise was about** — the cost of
meshing the *shipped* replay world, where water's 131 surface voxels and their
edges become real quads. That path is `mc_sim::replay::mesh_all` over
`ReplayWorld`, and no bench in this workspace covers it. Stated rather than
built: adding one is not in this spec's scope, and inventing a benchmark inside
a gate-atomic phase is how a phase acquires a second reason to be red.
