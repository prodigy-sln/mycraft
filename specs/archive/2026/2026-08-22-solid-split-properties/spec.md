...
- **FR-2.6**: Water is drawn in the shipped world, and its meshed **area** is what
  an independent per-voxel walk of the same world says rather than a number copied
  from a run of the mesher. The world's **quad count** is not derivable that way
  and is committed as a tripwire instead — the two halves are worded separately
  because the difference between them is a real one, set out below.
  ...
  - FR-2.6-S3: WHEN the shipped replay world is meshed THE SYSTEM SHALL report the quad count committed for the current scene, a number minted only once FR-2.6-S1 and FR-2.6-S2 are green and never edited to reach green

**FR-2.6-S3 was amended on 2026-08-23, after pass-1 validation returned it as a
scenario gap.** As first written it required a quad count *"derived from an
independent walk of the same world rather than one snapshotted from a run of the
mesher"*, and the test mapped to it compares against a committed number whose own
doc comment says it verifies nothing. The reviewer was right that the mapping was
unmet. **The scenario was what was wrong, and the reasoning matters more than the
ruling.**

**A quad count is a property of the merge, not of the world.** The mesher emits
the scanline-greedy decomposition of the visible faces — grow a run along the
primary axis, then extend it along the secondary while a whole row matches — and
that is deliberately not the fewest rectangles covering the same faces. A merger
growing columns before rows would be equally correct and would report a
*different count for identical geometry*. So "a quad count derived from an
independent walk" is not a hard thing to build: **it is not well-defined.** Any
derivation that agreed with the mesher would have to make the same ordering
choices, at which point it is a copy of its subject — what
`standards/global/testing.md` §2 forbids, and what this spec has already refused
twice on the ground that an oracle asking its subject's question is not an oracle.

**This is a contradiction between a scenario and the project's own standard, not
a requirement loosened because implementing it got hard.** The distinction is
worth stating plainly because the two look identical from outside. Nothing that
*was* derived stops being derived: FR-2.6-S1 and FR-2.6-S2 compare the mesher
against a per-voxel walk sharing no code with it and commit no number at all, and
FR-2.5-S1's per-direction figures are counted from the heightmap. Only the one
quantity that never had an oracle to be compared against is restated as what it
has been since it was minted.

**Four instruments guard the mesh, with four different reaches, each able to fail
on its own:**

- **Area** — the independent per-voxel walk (FR-2.6-S1, FR-2.6-S2) and FR-2.5-S1's
  per-direction figures. These see any change in *which faces are visible*, in
  aggregate.
- **Merge shape** — the property tests in `crates/mc-world/tests/mesh_properties.rs`,
  which pin the quads to an **exact partition** of the visible-face set: every
  visible face covered, no covered face invisible, no face covered twice, over
  randomly generated section contents and all six surroundings, **per face and per
  position**. Strictly stronger than any area sum, because it catches a face
  relocated with its area preserved, which a sum cannot see.
- **What the camera sees** — the golden set, judging the pixels the declared
  captures actually produce.
- **The quad count** — the committed tripwire, which makes a change of merge
  strategy land at that test with its remedy attached rather than surfacing later
  as a performance question nobody traced.

**The golden set is not the merge-shape witness, and the correction that produced
this amendment was of a proposal that it was.** A re-partition of the same visible
faces is **pixel-neutral by design**: a corner's texture coordinates are derived
from its own position under a repeating sampler, so one four-block quad shows the
texture four times rather than stretched once, and four one-block quads emit the
same texels at the same depths. Perceptual comparison against a disagreement
budget could not see the difference if there were one. The day merge shape becomes
visible — ambient occlusion, which is per-vertex, or PRO-952's translucency — the
goldens *do* become a second witness, and that is the day the tripwire's own
message about stale goldens becomes literally true rather than conditionally so.

**So the residual is named, and it is not a hole.** A merge change is one of two
things. Either it keeps the partition — then it is area-neutral and pixel-neutral
by construction and observable only as a count: not a defect, but a strategy
change somebody owes an explanation for, which is exactly what the tripwire
extracts. Or it breaks the partition — then the property tests redden directly and
name the face. There is no third case in which something a stakeholder can observe
